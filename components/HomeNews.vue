  ...
  <section id="section-news">
    <div class="container">
      ...
      <ul class="columns is-wrap is-mobile">
        <span class="is-hidden-touch btn btn-prev" :title="$t('前一页')" @click="prevNews">
        ...
        <li class="column is-full-mobile is-one-third-tablet" v-for="(item, index) in currentNews" :key="index">
          <nuxt-link :to="`/${$store.state.locale}/about/news/${item.id}`">
        ...
        <span class="is-hidden-touch btn btn-next" :title="$t('后一页')" @click="nextNews">
      ...
      <div class="more has-text-centered">
        <nuxt-link :to="`/${$store.state.locale}/about/news`">
          {{$t('更多')}}
